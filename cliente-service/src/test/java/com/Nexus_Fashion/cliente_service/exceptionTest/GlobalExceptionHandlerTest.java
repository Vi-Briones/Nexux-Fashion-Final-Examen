package com.Nexus_Fashion.cliente_service.exceptionTest;
...
import com.Nexus_Fashion.cliente_service.exception.ApiErrorResponse;
import com.Nexus_Fashion.cliente_service.exception.BadRequestException;
import com.Nexus_Fashion.cliente_service.exception.GlobalExceptionHandler;
...
public class GlobalExceptionHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        when(request.getRequestURI()).thenReturn("/clientes/test");
    }

    @Test
    void testHandleBadRequest_retorna400() {
        BadRequestException ex = new BadRequestException("Correo ya registrado");
 
        ResponseEntity<ApiErrorResponse> response = handler.handleBadRequest(ex, request);
 
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        assertEquals(400, response.getBody().getStatus());
        assertEquals("Correo ya registrado", response.getBody().getMessage());
        assertEquals("/clientes/test", response.getBody().getPath());
    ...
    void testHandleNotFound_retorna404() {
        ResourceNotFoundException ex = new ResourceNotFoundException("Cliente no encontrado con ID: 99");
 
        ResponseEntity<ApiErrorResponse> response = handler.handleNotFound(ex, request);
 
        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
        assertEquals(404, response.getBody().getStatus());
        assertEquals("Cliente no encontrado con ID: 99", response.getBody().getMessage());
    }
