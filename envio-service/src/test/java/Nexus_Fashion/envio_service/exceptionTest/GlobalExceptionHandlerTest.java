package Nexus_Fashion.envio_service.exceptionTest;
...
import Nexus_Fashion.envio_service.exception.ApiErrorResponse;
import Nexus_Fashion.envio_service.exception.BadRequestException;
import Nexus_Fashion.envio_service.exception.GlobalExceptionHandler;
import Nexus_Fashion.envio_service.exception.ResourceNotFoundException;
...
public class GlobalExceptionHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        when(request.getRequestURI()).thenReturn("/envios/test");
    }

    @Test
    void testHandleBadRequest_retorna400() {
        BadRequestException ex = new BadRequestException("Envio ya registrado");
 
        ResponseEntity<ApiErrorResponse> response = handler.handleBadRequest(ex, request);
 
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        assertEquals(400, response.getBody().getStatus());
        assertEquals("Envio ya registrado", response.getBody().getMessage());
        assertEquals("/envios/test", response.getBody().getPath());
    ...
    void testHandleNotFound_retorna404() {
        ResourceNotFoundException ex = new ResourceNotFoundException("Envio no encontrado con ID: 99");
 
        ResponseEntity<ApiErrorResponse> response = handler.handleNotFound(ex, request);
 
        assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
        assertEquals(404, response.getBody().getStatus());
        assertEquals("Envio no encontrado con ID: 99", response.getBody().getMessage());
    }
