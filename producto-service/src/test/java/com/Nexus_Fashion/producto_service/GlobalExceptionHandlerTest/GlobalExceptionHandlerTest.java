package com.Nexus_Fashion.producto_service.GlobalExceptionHandlerTest;
...
import com.Nexus_Fashion.producto_service.exception.ApiErrorResponse;
import com.Nexus_Fashion.producto_service.exception.BadRequestException;
import com.Nexus_Fashion.producto_service.exception.GlobalExceptionHandler;
import com.Nexus_Fashion.producto_service.exception.ResourceNotFoundException;
...
public class GlobalExceptionHandlerTest {
    ...
    @BeforeEach
    void setUp() {
        when(request.getRequestURI()).thenReturn("/productos/test");
    }
 
    @Test
    void testHandleBadRequest_retorna400() {
        BadRequestException ex = new BadRequestException("Producto inválido");
 
        ResponseEntity<ApiErrorResponse> response = handler.handleBadRequest(ex, request);
 
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        assertEquals(400, response.getBody().getStatus());
        assertEquals("Producto inválido", response.getBody().getMessage());
        assertEquals("/productos/test", response.getBody().getPath());
    ...
    void testHandleNotFound_retorna404() {
        ResourceNotFoundException ex = new ResourceNotFoundException("Producto no encontrado con ID: 99");
